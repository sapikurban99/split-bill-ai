...
export default function SplitBillPage() {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (parsedData) {
                ...
                // Discount: handle all possible field names
                const discount = toNum(parsedData.discount ?? parsedData.diskon ?? parsedData.potongan ?? parsedData.promo ?? parsedData.voucher ?? parsedData.disc ?? 0);
                // Subtotal
                const subtotal = toNum(parsedData.subtotal ?? parsedData.sub_total ?? parsedData.subtotal_amount ?? 0) || itemsSubtotal;
                // Total
                const total = toNum(parsedData.total ?? parsedData.grand_total ?? parsedData.total_amount ?? parsedData.total_bayar ?? 0) || (subtotal - discount + tax + serviceCharge);
    ...
    const finalCalculation = useMemo(() => {
        ...
        // Safely compute tax+service-discount ratio
        const extraCharges = (billData.tax || 0) + (billData.service_charge || 0) - (billData.discount || 0);
                ...
                <section>
                    <div className="flex items-center justify-between gap-4 rounded-2xl bg-surface-dark p-1 border border-white/5">
                        <div className="flex-1 p-5 flex flex-col gap-3">
                            ...
                            <button
                                onClick={() => {
                                    setBillData({ items: [], discount: 0, tax: 0, service_charge: 0, subtotal: 0, total: 0 });
                                    setStep('verify');
                                }}
        ...
        <div className="animate-fade-in flex flex-col min-h-screen bg-background-dark">
            ...
            {billData && billData.items.length > 0 && (
                <div className="sticky bottom-0 bg-background-dark border-t border-white/5 p-4 pb-8 w-full max-w-md mx-auto">
                            ...
                            {taxMode === 'amount' ? (
                                <input
                                    ...
                                    onChange={e => {
                                        ...
                                        setBillData({ ...billData, tax, total: billData.subtotal - billData.discount + tax + billData.service_charge });
                                    }}
                                    placeholder="0"
                                />
                            ) : (
                            ...
                            {scMode === 'amount' ? (
                                <input
                                    ...
                                    onChange={e => {
                                        ...
                                        setBillData({ ...billData, service_charge: sc, total: billData.subtotal - billData.discount + billData.tax + sc });
                                    }}
                                    placeholder="0"
                                />
                            ) : (
                    ...
                    <div className="mb-3">
                        <div className="flex items-center justify-between mb-1">
                            <label className="text-[10px] text-slate-500 uppercase tracking-wider">Diskon</label>
                            <button
                                onClick={() => {
                                    if (discMode === 'amount') {
                                        setDiscMode('percent');
                                        setDiscPercent(billData.subtotal > 0 ? String(Math.round((billData.discount / billData.subtotal) * 100)) : '0');
                                    } else {
                                        setDiscMode('amount');
                                    }
                                }}
                        ...
                        {discMode === 'amount' ? (
                            <input
                                ...
                                onChange={e => {
                                    ...
                                    setBillData({ ...billData, discount: disc, total: billData.subtotal - disc + billData.tax + billData.service_charge });
                                }}
                                placeholder="0"
                            />
                        ) : (
                            <div className="relative">
                                <input
                                    ...
                                    onChange={e => {
                                        const pct = e.target.value;
                                        setDiscPercent(pct);
                                        const disc = Math.round(billData.subtotal * (Number(pct) || 0) / 100);
                                        setBillData({ ...billData, discount: disc, total: billData.subtotal - disc + billData.tax + billData.service_charge });
                                    }}
                                    placeholder="0"
                                />
                        ...
                        {discMode === 'percent' && (
                            <p className="text-[10px] text-slate-500 mt-1">= Rp {billData.discount.toLocaleString()}</p>
                        )}
                    </div>

                    {(billData.tax + billData.service_charge + billData.discount) > 0 && (
                        <div className="flex justify-between items-center mb-3 px-1">
                            <span className="text-primary/70 text-xs font-medium">Total</span>
